...
export function ConfessionWall({
  ...
  useEffect(() => {
    execute(async () => {
      const result = await confessionsApi.getConfessions(1, initialLimit);
  ...
  const handleLoadMore = async () => {
    ...
    execute(async () => {
      const result = await confessionsApi.getConfessions(nextPage, initialLimit);
  ...
  // Handle confession reaction (TODO: implement reactions endpoint)
  const handleReact = async (confessionId: number) => {
    // TODO: Add reactions API endpoint
    console.log('React to confession:', confessionId);
  };
