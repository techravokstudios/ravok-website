...
- [ ] Build Hollywood Confessions submission form + feed UI (see `06-content-engine/CONTEXT.md`)
- [ ] Build Confessions ticker widget for homepage

### Week 3:
- [ ] Build Portfolio page with venture cards
- [ ] Rebuild About page with Amanda's story (she fills in the "[SPECIFIC MOMENT]" placeholder — copy exists in website-copy-10-06-2025.docx, the Bite Me origin story)
- [ ] Build Our Model page with SPV explainer
- [ ] Build Contact page with 3 segmented inquiry types (Investor / Partnership / General)
- [ ] Build Creators page with waitlist form
- [ ] Build Partners page
- [ ] Build /insights page with Confessions feed as hero + article grid below
...
- [ ] Write and run all migrations (ventures, documents, updates, contacts, waitlist, articles, tags, confessions)
- [ ] Create Eloquent models with relationships
- [ ] Build public API endpoints (contact form, waitlist, articles, public ventures, confessions submit + feed)
- [ ] Build confessions moderation endpoints (admin approve/reject/feature)
...
## Phase 4: Content Engine + Hollywood Confessions (Weeks 7-8)
**Goal:** Amanda can publish thought leadership, AND Hollywood Confessions launches as the audience-first growth engine.

### Hollywood Confessions (Priority — this drives traffic):
- [ ] Build `confessions` database table and migration
- [ ] Build public API: feed, submit, react endpoints
- [ ] Build admin API: moderation queue, approve/reject/feature, Amanda's response
- [ ] Build `<ConfessionFeed />` — social-feed style display on /insights hero
- [ ] Build `<ConfessionSubmitForm />` — anonymous submission (no auth required)
- [ ] Build `<ConfessionCard />` — individual confession with reactions and category tags
- [ ] Build `<ConfessionFeatured />` — Amanda's picks with her public response
- [ ] Build confession moderation queue in admin panel
- [ ] Build compact confession ticker/preview widget for homepage
- [ ] Auto-moderation: profanity filter, spam detection, rate limiting
- [ ] Add confession categories (Development Hell, Shady Accounting, Gatekeeping, etc.)
...
- [ ] Build `/insights` listing page — Confessions feed at top, articles below
...
- [ ] Write and publish first 3 articles (using confessions as hooks for thought leadership)
- [ ] Submit sitemap to Google Search Console

### Weekly Digest Email:
- [ ] Build "Hollywood Confessions Weekly" email signup form (replaces generic newsletter)
- [ ] Build weekly digest email template — top confessions + Amanda's takes
- [ ] Schedule Friday send via Laravel scheduler

**Deliverable:** Live Insights page with Hollywood Confessions feed + blog articles + weekly digest signup. This is the audience magnet.
